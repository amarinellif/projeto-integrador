...
public class AgentController {
    ...
    public ResponseEntity<WarningDueDateDto> getProductBatches(@PathVariable long id, @RequestParam int daysUntil){
        return ResponseEntity.ok(warehouseService.getWarningDueDateBatchesByWarehouse(daysUntil,id));
    }

    //@GetMapping("fresh-products/agent/warning-duedate-batches/{id}")
    //public ResponseEntity<WarningDueDateDto> getWrongPlaceBaches(@PathVariable long id, @RequestParam int daysUntil){
    //    return ResponseEntity.ok(warehouseService.getWarningDueDateBatchesByWarehouse(daysUntil,id));
    //}

    @GetMapping("fresh-products/agent/warning-temp-section/{id}")
    public ResponseEntity<WrongTempDto> getWrongPlaceBaches(@PathVariable long id){
